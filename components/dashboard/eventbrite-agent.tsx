...
interface EventbriteAgentProps {
  onClose?: () => void;
}

export function EventbriteAgent({ onClose }: EventbriteAgentProps) {
  ...
  useEffect(() => {
    async function loadLog() {
      try {
        const res = await fetch('/api/agents/eventbrite');
        const data = await res.json();
        setLog(data.log);
        
        // Mark Ian's visit
        await fetch('/api/agents/eventbrite', {
          method: 'POST',
      ...
      <div className="fixed inset-0 bg-black/80 z-50 flex items-center justify-center p-4">
        <div className="card w-full max-w-4xl max-h-[80vh] flex flex-col overflow-hidden">
          <div className="p-8 text-center">
            ...
            <p className="text-muted">Loading Eventbrite Agent...</p>
          </div>
        </div>
      </div>
    );
      ...
      <div className="card w-full max-w-4xl max-h-[80vh] flex flex-col overflow-hidden">
        ...
        <div className="flex items-center justify-between px-8 py-6 border-b border-border flex-shrink-0">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h3 className="text-xl font-semibold">🎫 Eventbrite Agent</h3>
              <p className="text-sm text-muted">
                Status: {log.status === 'healthy' ? '✅ Healthy' : log.status === 'warning' ? '⚠️ Needs Attention' : log.status} • 
                Last run: {formatTimeAgo(log.lastRun)}
              </p>
            </div>
          </div>
          {onClose && (
        ...
        <div className="flex gap-2 p-3 border-b border-border overflow-x-auto flex-shrink-0">
          ...
          >
            Developments ({log.developments.length})
          </button>
          <button
          ...
          >
            Brainstorming ({log.brainstorming.length})
          </button>
        </div>
            ...
            <div className="space-y-6">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <p className={`text-lg font-medium ${
                      ...
                      {log.status === 'healthy' ? '✅ Healthy' : log.status === 'warning' ? '⚠️ Needs Attention' : log.status}
                    </p>
                  </div>
                  <div>
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                ...
                <div className="p-4 rounded-xl bg-slate-900/50 border border-border/50 text-center">
                  ...
                  <p className="text-xs text-muted mt-1">Developments</p>
                </div>
                <div className="p-4 rounded-xl bg-slate-900/50 border border-border/50 text-center">
            ...
            <div className="space-y-4">
              ...
              <div className="flex gap-2">
                <input
                  ...
                  placeholder="Add new instruction for Eventbrite agent..."
          ...
          {activeTab === 'developments' && (
            <div className="space-y-2">
              {log.developments.length > 0 ? (
                ...
                <p className="text-muted text-sm">No developments yet.</p>
              )}
            </div>
          )}
          ...
          {activeTab === 'brainstorming' && (
            <div className="space-y-2">
              {log.brainstorming.length > 0 ? (
                ...
                <p className="text-muted text-sm">No brainstorming sessions yet. Decisions made with Rook will appear here.</p>
              )}
            </div>
          )}
        </div>
